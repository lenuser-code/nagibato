 ...
 * @prop {boolean} isLocked - このボタンの操作が封じられているか
 * @prop {boolean} active - (stdgam.Sceneの意味で) このオブジェクトが有効か
 */
let PhysicalButton = class{
    static #images = new stdgam.CachePool();

    static getImage(color){
        const img = this.#images.get(color);
        if(img) return img;
        const CB = new stdgam.ColorBox(
            { bg: color, bgBlend: "#444", lighting: "#888",
              border: "gray", borderInner: "white", borderBlur: "rgb(200,200,200,0.2)" },
            (ctx, w, h, lw, opt) => {
                ctx.beginPath();
                ctx.arc(23+lw/2, 22+lw/2, 22, 0, Math.PI * 2);
            });
        return CB.make(this.#images, color, 50, 50, 4);
    }

    constructor(x, y, label, key){
        this.x = x;
        this.y = y;
        this.label = label;
        this.key = key;
    ...
    draw(GE, ctx){
        ...
        // ボタン本体（円）
        const img = PhysicalButton.getImage(this.isPressed ? "#aaa" : "#ff0");
        ctx.drawImage(img, -25, -24);

        // ラベル文字（A, S, D）
        ctx.fillStyle = this.isPressed ? "white" : "#a84";
    ...
    execute(GE){
        ...
        this.isPressed = !this.locked && GE.input[fn](this.key);
        return true;
    }
}
 ...
 * @return {PhysicalButton} 生成されたコンポーネント
 */
let createPhysicalButton = function(x, y, key, label) {
    return new PhysicalButton(x, y, label, key);
}
 ...
 * 入力受付はmainSceneで行う.
 * @returns {stdtask.AcceptKey} 生成されたコンポーネント
 */
